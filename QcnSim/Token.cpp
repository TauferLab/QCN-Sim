...
void Token::updateHopsFromExplicitRoute(std::shared_ptr<Entity> currentHop) {
	...
	// Fetch the next hop from Route object attached to token. If it is nullptr, then there is no route. Must treat this in caller.
 ...
 *
 * Note that, if comparisons are being made with shared pointers, this function will actually not
 * be used. The shared pointers will perform the comparisons "internally."
 * 
 ...
 *
 * @details 
 * This is the overloading of the comparison != operator for use with token.  The result should be FALSE
 * if the left token has the same relevant contents as the right token.
 * It should suffice to compare the unique IDs, but let's just do it the redundant way.
 *
 * Note that, if comparisons are being made with shared pointers, this function will actually not
 * be used. The shared pointers will perform the comparisons "internally."
 *
 * Also note that this function is simple the negation of == operator. It is being implemented for the sake of 
